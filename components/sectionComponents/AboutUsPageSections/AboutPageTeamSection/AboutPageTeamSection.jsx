...
const AboutPageTeamSection = () => {
    ...
    useEffect(() => {
        axios 
        .get(`${process.env.NEXT_PUBLIC_BASE_URL}/about_page_team?locale=en`)
        .then((res) => {
        ...
        <div className='pt-20 mb-20 relative'>
            ...
            <div className='absolute top-60 z-[-1] left-0 lg:block hidden dark:hidden'>
                <svg xmlns="http://www.w3.org/2000/svg" width="1440" height="437" viewBox="0 0 1440 437" fill="none">
                    <path d="M-144.621 57.0626C-23.6867 8.27523 249.232 -54.8088 445.758 95.6514C691.415 283.727 824.064 267.537 1040.75 198.464C1222.42 140.552 1475.7 344.988 1520.98 436.536" stroke="white" stroke-opacity="0.7" strokeDasharray="8 8" />
                </svg>
            </div>
            <div className='absolute bottom-20 z-[-1] left-0 lg:block hidden dark:hidden'>
                <svg xmlns="http://www.w3.org/2000/svg" width="1440" height="205" viewBox="0 0 1440 205" fill="none">
                    <path d="M-113 113.708C-2.13898 45.0418 256.053 -63.5916 475.328 51.2084C749.422 194.708 877.377 156.168 1079.14 51.2084C1248.3 -36.791 1532.68 121.534 1592.89 204.034" stroke="white" stroke-opacity="0.7" strokeDasharray="8 8" />
                </svg>
            </div>
            <div>
                <h5 className='text-white section-heading text-center pt-12'>{data?.title}</h5>
                <p className='text-white text-xl md:text-2xl lg:text-[22px] text-center max-w-[614px] mx-auto leading-tight mt-5'>{data?.subtitle}</p>
                <div className='flex flex-wrap mt-20 gap-5 gap-y-20 justify-center'>
                    {
                        data?.members?.map((member, index) =>
                            <div key={index} className={`bg-lightGray max-w-[230px] h-max ${index % 2 !== 0 ? '' : 'mt-12'}`}>
                                <Image src={member?.img?.url} className='w-full' width={228} height={239} alt='' />
                                <div className='bg-white pb-3 relative'>
                                    <Link href={member?.linkedin_id} className='absolute flex justify-center w-full -top-5'>
                                    ...
                                    <p className=' text-3xl pt-6 text-primary text-center font-title leading-tight'>{member?.name}</p>
                                    <p className='text-base text-primary text-center font-title'>{member?.position}</p>
                                </div>
                            </div>
                        )
                    }
